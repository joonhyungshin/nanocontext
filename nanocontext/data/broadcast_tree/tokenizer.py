import numpy as np

from nanocontext.utils import d_order, d_divide, get_numpy_rng

from .tree import (
    OrderedTree, BroadcastTree, LazyBroadcastTree, dynamic_broadcast_tree, BroadcastConfig,
    block_autoregressive_tree
)


class SpinTreeTokenizer:
    def __init__(self, max_vocab_size, bos_token=0, neg_token=1, pos_token=2, punc_token_base=3):
        self.max_vocab_size = max_vocab_size
        self.bos_token = bos_token
        self.neg_token = neg_token
        self.pos_token = pos_token
        self.punc_token_base = punc_token_base

    def punctuation(self, subtree_height):
        token = subtree_height + self.punc_token_base - 1
        if token >= self.max_vocab_size:
            raise ValueError("max_vocab_size is too small")
        return token

    def is_punc_token(self, token):
        return token >= self.punc_token_base

    def subtree_height(self, punc_token):
        return punc_token - self.punc_token_base + 1

    def spin_token(self, spin):
        return self.neg_token if spin < 0 else self.pos_token

    def sign(self, spin_token):
        return -1 if spin_token == self.neg_token else 1

    def tokenize_stream(self, tree: OrderedTree | BroadcastTree, prepend_bos=False):
        if prepend_bos:
            yield self.bos_token
        for idx, spin in enumerate(tree.get_leaves()):
            if idx > 0:
                zero_cnt = d_order(idx, tree.d)
                if zero_cnt > 0:
                    yield self.punctuation(zero_cnt)
            yield self.spin_token(spin)

    def tokenize(self, tree: OrderedTree | BroadcastTree, prepend_bos=False):
        return list(self.tokenize_stream(tree, prepend_bos=prepend_bos))

    def tokenize_lazy_stream(self, tree: LazyBroadcastTree, batch_height, prepend_bos=False):
        if prepend_bos:
            yield self.bos_token, None, None
        leaf_idx = 0
        for subtree, ancestors in tree.sample_subtree_stream(0, 0, batch_height):
            if leaf_idx > 0:
                zero_cnt = d_order(leaf_idx, tree.d)
                if zero_cnt > 0:
                    yield self.punctuation(zero_cnt), subtree, ancestors
            for token in self.tokenize_stream(subtree, prepend_bos=False):
                yield token, subtree, ancestors
            leaf_idx += subtree.num_leaves

    def decode_trees(self, tokens):
        # Make bos_token in the beginning optional... (due to summary)
        current_tree = OrderedTree()
        current_node = current_tree.root
        for token in tokens:
            if token == self.bos_token:
                if not current_tree.is_singleton():
                    yield current_tree
                current_tree = OrderedTree()
                current_node = current_tree.root
            elif token in [self.neg_token, self.pos_token]:
                current_node.create_child(self.sign(token))
            elif self.is_punc_token(token):
                jump_height = self.subtree_height(token)
                for i in range(jump_height):
                    current_node, created = current_node.get_parent_or_create()
                    if created:
                        current_tree.root = current_node
                for i in range(jump_height):
                    current_node = current_node.create_child()
        yield current_tree


class SummaryTokenizer(SpinTreeTokenizer):
    def __init__(self, max_vocab_size, bos_token=0, neg_token=1, pos_token=2, punc_token_base=5,
                 summary_start_token=3, summary_end_token=4):
        super().__init__(max_vocab_size, bos_token=bos_token, neg_token=neg_token, pos_token=pos_token,
                         punc_token_base=punc_token_base)
        self.summary_start_token = summary_start_token
        self.summary_end_token = summary_end_token

    def tokenize_summary_stream(self, summary, config: BroadcastConfig):
        raise NotImplementedError

    def tokenize_summary(self, summary, config: BroadcastConfig, wrap=True):
        tokens = list(self.tokenize_summary_stream(summary, config))
        if wrap:
            tokens = [self.summary_start_token] + tokens + [self.summary_end_token]
        return tokens

    def init_summary(self, config: BroadcastConfig):
        raise NotImplementedError

    def update_summary(self, summary, token_data, config: BroadcastConfig):
        raise NotImplementedError

    def tokenize_with_summary_stream(self, tree: LazyBroadcastTree, batch_height, summary_indices, prepend_bos=False):
        summary = self.init_summary(tree.config)
        tokens = []
        for token_idx, token_data in enumerate(self.tokenize_lazy_stream(tree, batch_height, prepend_bos=prepend_bos)):
            if token_idx in summary_indices:
                yield tokens, self.tokenize_summary(summary, tree.config)
                tokens.clear()
            tokens.append(token_data[0])
            summary = self.update_summary(summary, token_data, tree.config)
        yield tokens, self.tokenize_summary(summary, tree.config)

    def init_summary_tokens(self, config: BroadcastConfig):
        return self.tokenize_summary(self.init_summary(config), config)


class SegmentSummaryTokenizer(SummaryTokenizer):
    def __init__(self, max_vocab_size, bos_token=0, neg_token=1, pos_token=2, punc_token_base=8,
                 summary_start_token=3, summary_end_token=4, summary_neg_token=5, summary_pos_token=6,
                 summary_pad_token=7):
        super().__init__(max_vocab_size, bos_token=bos_token, neg_token=neg_token, pos_token=pos_token,
                         punc_token_base=punc_token_base, summary_start_token=summary_start_token,
                         summary_end_token=summary_end_token)
        self.summary_neg_token = summary_neg_token
        self.summary_pos_token = summary_pos_token
        self.summary_pad_token = summary_pad_token

    def punctuation(self, subtree_height, summary=False):
        token = subtree_height * 2 + self.punc_token_base
        if summary:
            token += 1
        if self.max_vocab_size <= token:
            raise ValueError("max_vocab_size too small")
        return token

    def is_punc_token(self, token):
        return token >= self.punc_token_base

    def subtree_height(self, punc_token):
        return (punc_token - self.punc_token_base) // 2

    def spin_token(self, spin, summary=False):
        pos_token = self.summary_pos_token if summary else self.pos_token
        neg_token = self.summary_neg_token if summary else self.neg_token
        return neg_token if spin < 0 else pos_token

    def sign(self, spin_token):
        return -1 if spin_token in [self.neg_token, self.summary_neg_token] else 1

    def tokenize_summary_stream(self, summary, config, wrap_to=None):
        summary_data, _ = summary
        wrap_to = wrap_to or config.d
        for summary_height, summary_spins in summary_data:
            yield self.punctuation(summary_height, summary=True)
            for spin in summary_spins:
                yield self.spin_token(spin, summary=True)
            for _ in range(wrap_to - len(summary_spins)):
                yield self.summary_pad_token

    def init_summary(self, config):
        ctx = {
            "cur_subtree": None,
            "cur_subtree_leaf_idx": 0,
            "leaf_idx": 0,
            "last_ancestors": []
        }
        return [(config.height - 1 - i, []) for i in range(config.height)], ctx

    def update_summary(self, summary, token_data, config):
        token, subtree, ancestors = token_data
        summary_data, ctx = summary
        if ctx["cur_subtree"] is not subtree:
            ctx["cur_subtree"] = subtree
            ctx["cur_subtree_leaf_idx"] = 0
        if token == self.bos_token:
            pass
        elif token in [self.pos_token, self.neg_token]:
            summary_data[-1][1].append(self.sign(token))
            ctx["cur_subtree_leaf_idx"] += 1
            ctx["leaf_idx"] += 1
        elif self.is_punc_token(token):
            punc_height = self.subtree_height(token)
            for i in range(punc_height):
                summary_data[config.height - 1 - i][1].clear()
            if ctx["cur_subtree_leaf_idx"] == 0:
                height = d_order(ctx["leaf_idx"], config.d)
                assert height == punc_height
                summary_spin = ctx["last_ancestors"][config.height - punc_height - 1]
            else:
                idx, height = d_divide(ctx["cur_subtree_leaf_idx"], config.d)
                assert height == punc_height
                summary_spin = subtree.values_at(subtree.height - height)[idx - 1]
            summary_data[config.height - 1 - punc_height][1].append(summary_spin)
        else:
            assert False
        ctx["last_ancestors"] = ancestors
        return summary


class HierarchySummaryTokenizer(SummaryTokenizer):
    def __init__(self, max_vocab_size, bos_token=0, neg_token=1, pos_token=2, punc_token_base=8,
                 index_token_base=9, summary_start_token=3, summary_end_token=4,
                 summary_neg_token=5, summary_pos_token=6, summary_pad_token=7):
        super().__init__(max_vocab_size, bos_token=bos_token, neg_token=neg_token, pos_token=pos_token,
                         punc_token_base=punc_token_base, summary_start_token=summary_start_token,
                         summary_end_token=summary_end_token)
        self.index_token_base = index_token_base
        self.summary_neg_token = summary_neg_token
        self.summary_pos_token = summary_pos_token
        self.summary_pad_token = summary_pad_token

    def punctuation(self, subtree_height):
        token = self.punc_token_base + subtree_height * 2
        if token >= self.max_vocab_size:
            raise ValueError("max_vocab_size too small")
        return token

    def subtree_height(self, punc_token):
        return (punc_token - self.punc_token_base) // 2

    def spin_token(self, spin, summary=False):
        pos_token = self.summary_pos_token if summary else self.pos_token
        neg_token = self.summary_neg_token if summary else self.neg_token
        return neg_token if spin < 0 else pos_token

    def sign(self, spin_token):
        return -1 if spin_token in [self.neg_token, self.summary_neg_token] else 1

    def is_punc_token(self, token):
        return token >= self.punc_token_base and (token - self.punc_token_base) % 2 == 0

    def sibling_index_token(self, idx):
        token = self.index_token_base + idx * 2
        if token >= self.max_vocab_size:
            raise ValueError("max_vocab_size too small")
        return token

    def tokenize_summary_stream(self, summary, config):
        summary_data, ctx = summary
        for spin, node_index in summary_data:
            yield self.summary_pad_token if spin == 0 else self.spin_token(spin, summary=True)
            yield self.sibling_index_token(node_index)

    def init_summary(self, config: BroadcastConfig):
        ctx = {
            "cur_subtree": None,
            "cur_subtree_leaf_idx": 0,
        }
        return [(0, 0) for _ in range(config.height)], ctx

    def update_summary(self, summary, token_data, config: BroadcastConfig):
        token, subtree, ancestors = token_data
        summary_data, ctx = summary
        if ctx["cur_subtree"] is not subtree:
            ctx["cur_subtree"] = subtree
            ctx["cur_subtree_leaf_idx"] = 0
        if token == self.bos_token:
            pass
        elif token in [self.neg_token, self.pos_token]:
            full_ancestors = ancestors + subtree.get_ancestors(ctx["cur_subtree_leaf_idx"])
            for i in range(config.height):
                depth = config.height - 1 - i
                parent_spin, sibling_idx = summary_data[depth]
                if parent_spin == 0:
                    parent_spin = full_ancestors[depth]
                    summary_data[depth] = (parent_spin, sibling_idx)
                else:
                    break
            ctx["cur_subtree_leaf_idx"] += 1
            parent_spin, sibling_idx = summary_data[-1]
            summary_data[-1] = (parent_spin, sibling_idx + 1)
        elif self.is_punc_token(token):
            punc_height = self.subtree_height(token)
            for i in range(punc_height):
                summary_data[config.height - 1 - i] = (0, 0)
            spin, sibling_idx = summary_data[config.height - 1 - punc_height]
            summary_data[config.height - 1 - punc_height] = (spin, sibling_idx + 1)
        else:
            assert False
        return summary_data, ctx


def tokenized_broadcast_trees(config, batch_height, tokenizer, seed=None):
    rng = get_numpy_rng(seed, local=True)
    while True:
        tree = dynamic_broadcast_tree(config, batch_height, seed=rng)
        for leaf_idx, subtree, _ in tree:
            tokens = []
            if leaf_idx == 0:
                tokens.append(tokenizer.bos_token)
            else:
                zero_cnt = d_order(leaf_idx, config.d)
                tokens.append(tokenizer.punctuation(zero_cnt))
            tokens.extend(tokenizer.tokenize(subtree))
            yield tokens


def tokenized_block_autoregressive_trees(config, batch_height, tokenizer, seed=None):
    rng = get_numpy_rng(seed, local=True)
    while True:
        tree_sequence = block_autoregressive_tree(config, batch_height, seed=rng)
        leaf_idx = 0
        for tree in tree_sequence:
            tokens = []
            if leaf_idx == 0:
                tokens.append(tokenizer.bos_token)
            else:
                zero_cnt = d_order(leaf_idx, config.d)
                tokens.append(tokenizer.punctuation(zero_cnt))
            leaf_idx += tree.num_leaves
            tokens.extend(tokenizer.tokenize(tree))
            yield tokens


def num_tokens_expected(tree, prepend_bos=False):
    res = tree.d ** (tree.height - 1) * (tree.d + 1)
    if not prepend_bos:
        res -= 1
    return res


def tokenized_broadcast_trees_with_summaries(config, batch_height, tokenizer: SummaryTokenizer, summary_every,
                                             seed=None):
    rng = get_numpy_rng(seed, local=True)
    tokens_window = []
    beginning = True
    num_trees = 0
    while True:
        tree = LazyBroadcastTree(config, seed=rng)
        num_tokens = num_tokens_expected(tree, prepend_bos=not beginning)
        if beginning:
            summary_indices = range(0, num_tokens, summary_every)
        else:
            start_idx = summary_every - len(tokens_window)
            summary_indices = range(start_idx, num_tokens, summary_every)
        for tokens, summary in tokenizer.tokenize_with_summary_stream(tree, batch_height, summary_indices,
                                                                      prepend_bos=not beginning):
            tokens_window.extend(tokens)
            if len(tokens_window) % summary_every == 0:
                yield num_trees, tokens_window, summary
                tokens_window = []
        beginning = False
        num_trees += 1

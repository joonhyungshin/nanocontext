...
class BroadcastTreeEngine:
    def __init__(self, tokenizer: SpinTreeTokenizer, sampler, stateful=False):
    ...
    def generate_tree_raw_stateful_stream(self, prompt, num_samples=1, allow_many=False, max_states=None, **kwargs):
    ...
    def generate_tree_raw_stateful(self, prompt, max_tokens, num_samples=1, allow_many=False, **kwargs):
        summary_len = len(prompt)
        content_len = self.sampler.context_len - 2 * summary_len
        ...
        num_tokens = 0
        for tokens_tensor in self.generate_tree_raw_stateful_stream(prompt, num_samples=num_samples,
    ...
    def generate_tree_stateful(self, prompt, num_samples=1, max_tokens=None, **kwargs):
        tree_tokens = [[] for _ in range(num_samples)]
        completed = [False for _ in range(num_samples)]
        for content_tokens in self.generate_tree_raw_stateful_stream(prompt, num_samples=num_samples, **kwargs):
            if all(completed):
                break
    ...
    def generate_tree_raw_simple(self, prompt, max_tokens, num_samples=1, allow_many=False, **kwargs):
        end_token = None if allow_many else self.tokenizer.bos_token
        return self.sampler.generate_batch_tensor(prompt, max_tokens,
                                                  num_samples=num_samples, end_token=end_token, **kwargs)

    def generate_tree_simple(self, prompt, num_samples=1, **kwargs):
        tokens = self.sampler.generate_batch(prompt, num_samples=num_samples, end_token=self.tokenizer.bos_token,
                                             **kwargs)
        trees = [next(self.tokenizer.decode_trees(tree_token)) for tree_token in tokens]
        return trees

    def generate_tree_raw(self, *args, **kwargs):
        if self.stateful:
            return self.generate_tree_raw_stateful(*args, **kwargs)
        else:
            return self.generate_tree_raw_simple(*args, **kwargs)

    def generate_tree(self, *args, **kwargs):
        if self.stateful:
            return self.generate_tree_stateful(*args, **kwargs)
        else:
            return self.generate_tree_simple(*args, **kwargs)
